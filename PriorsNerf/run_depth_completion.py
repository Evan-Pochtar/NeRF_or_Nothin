...
from util import MeanTracker
import cv2

from data import ScanNetDataset, convert_depth_completion_scaling_to_m, create_random_subsets
from util import print_network_info, get_hours_mins, MeanTracker, make_image_grid, apply_max_filter, update_learning_rate
...
class Validator:
    def __init__(self, val_dataset, unnormalize, min_val_rmse, device):
        ...
        if val_sample_count < validate_on_at_least_n_samples:
            ...
            print("Create {} validation subsets with length {} or {}".format(len(self.val_subsets), len(self.val_subsets[0]), len(self.val_subsets[-1])))
    ...
    def validate(self, net, optimizer, args, tb, epoch, step):
        with torch.no_grad():
            ...
            for i, data in enumerate(DataLoader(dataset=self.val_subsets[self.next_subset_index()], batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=True)):
